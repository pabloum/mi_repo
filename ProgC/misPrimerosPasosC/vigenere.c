...
char get_op(char s[]);
void menu();
char* get_char(char s[]); // esta función no admite que el usuario ingrese espacios dentro de la cadena de caracteres
...
int main (){
    ...
    while(1){
        ...
        switch (op){
            ...
            case '2':
                system("cls");
                printf("So, we are going to read\n\n");
                // Ask for the message
...
// I count the length of the message
    ...
    int cont=0;
    if (sizeM>sizeP){
        for(int i=sizeP;i<sizeM;i++){
            if(msn[i]==' '){
                pass[i]=' ';
                printf("El componente de la posicion %d es es %c\n\n",i,msn[i]);
            }else {
                pass[i]=pass[cont];
                cont++;
                if(cont==sizeP){
